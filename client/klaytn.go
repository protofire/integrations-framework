...
func (e *KlaytnClient) ProcessTransaction(txHash common.Hash) error {
	var txConfirmer HeaderEventSubscription
	if e.Network.Config().MinimumConfirmations == 0 {
		txConfirmer = &KlaytnInstantConfirmations{}
	} else {
		txConfirmer = NewKlaytnTransactionConfirmer(e, txHash, e.Network.Config().MinimumConfirmations)
	}
...
// TransactionCallMessage returns a filled Klaytn CallMsg object with suggest gas price and limit
func (e *KlaytnClient) TransactionCallMessage(
	from BlockchainWallet,
	to common.Address,
	value *big.Int,
	data []byte,
...
func (e *KlaytnClient) TransactionOpts(
	from BlockchainWallet,
	to common.Address,
	value *big.Int,
	data []byte,
	...
	opts, err := klaytnextended.NewKeyedTransactorWithChainID(privateKey, chID)
	if err != nil {
		return nil, err
	}
...
func (e *KlaytnClient) isTxConfirmed(txHash common.Hash) (bool, error) {
	...
	if !isPending {
		...
		block, err := e.Client.BlockByHash(context.Background(), txHash)
		if err != nil {
			return !isPending, err
		}
		if receipt.Status == 0 {
			log.Warn().Str("TX Hash", txHash.Hex()).Msg("Transaction failed and was reverted!")
			reason, err := e.errorReason(e.Client, tx, block)
			if err != nil {
				return false, err
			}
...
func (e *KlaytnClient) errorReason(
	...
	signer := types.NewEIP155Signer(chID)
	...
	res, err := b.CallContract(context.Background(), callMsg, block.Number())
	if err != nil {
...
// confirmations
func NewKlaytnTransactionConfirmer(eth *KlaytnClient, txHash common.Hash, minConfirmations int) *KlaytnTransactionConfirmer {
