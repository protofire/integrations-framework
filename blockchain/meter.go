...
// Handles specific issues with the Meter EVM chain: https://docs.meter.io/

// MeterMultinodeClient represents a multi-node, EVM compatible client for the Meter network
type MeterMultinodeClient struct {
	*EthereumMultinodeClient
}

// MeterClient represents a single node, EVM compatible client for the Meter network
type MeterClient struct {
	*EthereumClient
}

// NewMeterClient returns an instantiated instance of the Meter client that has connected to the server
func NewMeterClient(networkSettings *config.ETHNetwork) (EVMClient, error) {
	client, err := NewEthereumClient(networkSettings)
	log.Info().Str("Network Name", client.GetNetworkName()).Msg("Using custom Meter client")
	return &MeterClient{client.(*EthereumClient)}, err
}

// NewMeterMultinodeClient returns an instantiated instance of all Meter clients connected to all nodes
func NewMeterMultiNodeClient(
	_ string,
	...
	for idx, networkURL := range networkSettings.URLs {
		...
		ec, err := NewMeterClient(networkSettings)
		if err != nil {
			return nil, err
		}
		ec.SetID(idx)
	...
	return &MeterMultinodeClient{multiNodeClient}, nil
}

// Fund sends some ETH to an address using the default wallet
func (m *MeterClient) Fund(toAddress string, amount *big.Float) error {
	privateKey, err := crypto.HexToECDSA(m.DefaultWallet.PrivateKey())
	to := common.HexToAddress(toAddress)
	if err != nil {
		return fmt.Errorf("invalid private key: %v", err)
	}
	// Meter uses legacy transactions and gas estimations, is behind London fork as of 05/04/2022
...
func (m *MeterClient) DeployContract(
	contractName string,
	...
	// Meter uses legacy transactions and gas estimations, is behind London fork as of 04/27/2022
	suggestedGasPrice, err := m.Client.SuggestGasPrice(context.Background())
	if err != nil {
		return nil, nil, nil, err
	}
	...
	log.Info().
		...
		Str("From", m.DefaultWallet.Address()).
		Str("Total Gas Cost (MTR)", utils.WeiToEther(transaction.Cost()).String()).
