...
func (o setConfigEncodedComponents) encode() []byte {
	...
	if len(rv) > configSizeBound {
		_err = "config serialization too large"
		panic(_err)
	}
	return rv
}
...
func checkFieldNamesAgainstStruct(fields map[string]bool, i interface{}) {
	...
	for i := 0; i < s.NumField(); i++ {
		...
		if !fields[fieldName] {
			_err = "no encoding found for " + fieldName
			panic(_err)
		}
		fields[fieldName] = false
	}
	for name, unseen := range fields {
		_err = "extra field found in abiencode schema, " + name
		if unseen {
			panic(_err)
		}
	}
...
func init() { // check that abiencode fields match those of config structs
	...
	if essName != "sharedSecretEncryptions" {
		...
		_err = "expecting sharedSecretEncryptions in last position, got " + essName
		panic(_err)
	}
...
func checkFieldNamesMatch(s, t interface{}) {
	...
	if st.NumField() != tt.NumField() {
		var _err any
		_err = fmt.Sprintf("number of fields differ: %T has %d, %T has %d",
			s, st.NumField(),
			t, tt.NumField())
		panic(_err)
	}
	for i := 0; i < st.NumField(); i++ {
		if st.Field(i).Name != tt.Field(i).Name {
			var _err any
			_err = fmt.Sprintf("field name mismatch on %T vs %T: %s vs %s",
